...
#     for i in range(h):
#         for j in range(0, w - 1, 2):  # Swap every two adjacent pixels
#             image_array[i, j], image_array[i, j + 1] = image_array[i, j + 1], image_array[i, j]
#     return image_array

# def add_to_pixels(image_array, value):
#     return (image_array + value) % 256  # Ensure the pixel values remain within valid RGB range

# def encrypt_image(input_path, output_path):
#     # Load the image
#     image_array, mode = load_image(input_path)

#     # Encrypt the image
#     encrypted_image = swap_pixels(image_array.copy())
#     encrypted_image = add_to_pixels(encrypted_image, 50)  # Add 50 to each pixel value

#     # Save the encrypted image
#     save_image(encrypted_image, mode, output_path)

# def decrypt_image(input_path, output_path):
#     # Load the encrypted image
#     encrypted_image, mode = load_image(input_path)

#     # Decrypt the image
#     decrypted_image = add_to_pixels(encrypted_image.copy(), -50)  # Subtract 50 from each pixel value
#     decrypted_image = swap_pixels(decrypted_image)

#     # Save the decrypted image
#     save_image(decrypted_image, mode, output_path)
...
start1 = tk.Label(text="Encrypted\nDecrypted\nImage", font=("Arial", 30), fg="magenta")
start1.place(x=730, y=280)
